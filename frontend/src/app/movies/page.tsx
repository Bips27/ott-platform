...
export default function MoviesPage() {
  ...
  const fetchMovies = async () => {
    try {
      const res = await fetch(`/api/content/movies`);
      const json = await res.json();
      const items: MovieItem[] = (json.data || []).map((c: any) => ({
        id: c._id,
        title: c.title,
        ...
        genres: c.genres || [],
        duration: c.formattedDuration || `${Math.max(1, Math.round((c.duration || 0) / 60))}m`,
        rating: c.rating,
      }));
      setMovies(items);
    } catch (error) {
  ...
  const allGenres = useMemo(() => {
    // Prefer backend categories for dropdown; fallback to genres from items
  ...
  const filterByGenre = (items: MovieItem[], genre: string) => {
    if (genre === 'all') return items;
    return items.filter((m) => (m.genres || []).includes(genre));
  };

  const filtered = useMemo(() => filterByGenre(movies, selectedGenre), [movies, selectedGenre]);
