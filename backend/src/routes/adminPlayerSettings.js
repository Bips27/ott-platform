...
// Get player settings for a specific user (admin only)
router.get('/:userId', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  ...
  const settings = await PlayerSettings.getOrCreateSettings(userId);
...
// Update player settings for a specific user (admin only)
router.put('/:userId', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const { userId } = req.params;
...
// Reset player settings to defaults for a specific user (admin only)
router.post('/:userId/reset', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const { userId } = req.params;
...
// Get all player settings (admin only)
router.get('/', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const page = parseInt(req.query.page) || 1;
  const limit = parseInt(req.query.limit) || 10;
  const skip = (page - 1) * limit;
  
  const settings = await PlayerSettings.find()
    .populate('userId', 'email firstName lastName role')
    .skip(skip)
    .limit(limit)
    .sort({ updatedAt: -1 });
  
  const total = await PlayerSettings.countDocuments();
...
// Get available languages for subtitles and audio (admin only)
router.get('/languages', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const languages = [
...
// Get available quality options (admin only)
router.get('/quality-options', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
...
// Get available playback speed options (admin only)
router.get('/speed-options', authMiddleware, adminMiddleware, asyncHandler(async (req, res) => {
  const speedOptions = [
